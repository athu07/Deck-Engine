...
CASE_STUDY_RULES = (
    ...
    "Efficiency', 'Compliance Monitoring', 'Risk Management', 'Real-Time Data Access "
    "Enablement', 'Process Automation Implementation' or any '<Gerund> <Noun> <Gerund/label>' "
    "pattern — these are technology labels dressed as business outcomes, describing a "
    "FEATURE, not what the capability actually does for the business.\n"
    "- READS LIKE SOMETHING THAT HAPPENED, NOT A CATEGORY SUMMARY: a reader must be able to "
    "picture the actual situation inside a real company — the exact mechanical breakdown "
    "(what didn't connect to what, who was reconciling what by hand, what leverage was being "
    "left on the table), not a label for the general kind of problem. Test every sentence: "
    "does it name a SITUATION, or just NAME A CATEGORY of situation? "
    "'Faced challenges harmonizing procurement systems, causing data silos and "
    "inefficiencies' is a category label -- it could describe any of a thousand companies "
    "and tells the reader nothing that actually happened. 'Ran two entirely separate ERPs "
    "with no connectivity, duplicating supplier negotiations and reconciling spend manually "
    "at month-end' is a situation -- a reader can picture it. Same test for the solution: "
    "'implemented a unified platform enabling real-time data access' is a category claim; "
    ...
    "(see the PROOF-number rule below), it can still appear as a Results bullet on "
    ...
    "- NUMBERS IN CHALLENGE/SOLUTION -- TWO KINDS, TREATED DIFFERENTLY (owner-spec, "
    "2026-08-12, replacing a blanket ban that made every generated case study read as vague "
    "because it avoided ALL numbers, not just risky ones):\n"
    "  TEXTURE numbers -- team/headcount size, a rough duration (days/weeks/months), a volume "
    "count (invoices/month, tickets/week, sites, SKUs) -- MAY be invented realistically, the "
    "same license already given below for qualitative mechanism detail. These are scene-"
    "setting, like any anonymised case study's illustrative team size -- not numbers a client "
    "would fact-check or repeat externally. Make them SPECIFIC: 'a 12-person AP team', "
    "'invoices sat for 3 weeks before clearing', never 'the team' / 'took a while'.\n"
    "  PROOF numbers -- a dollar figure, a percentage, an ROI or cost-savings claim, a "
    "headcount ACTUALLY reduced -- may appear in the Challenge or Solution ONLY if genuinely "
    "present in the real source material provided (research, profile, notes, or the user's own "
    "pasted content) -- never estimated, rounded, or inferred from an industry benchmark to "
    "sound plausible. When no real proof number exists, add genuine QUALITATIVE specificity "
    "instead -- name the actual mechanism of the failure or the fix using facts already "
    "established elsewhere in the case study (e.g. 'watching diagnostic delays become routine "
    "-- not because radiologists were slow, but because the software's own interface made "
    "pulling up prior scans and case history slower than the read itself' -- specific and "
    "concrete, no invented stat). A wrong or fabricated PROOF number in a live client "
    "conversation is worse than an honest sentence with no proof-metric in it -- but a "
    "realistic TEXTURE number is not that risk, and withholding it is what makes a case study "
    "read as vague and made-up. This rule governs ONLY the Challenge and Solution fields -- it "
    "does not change how the separate Results bullets are produced.\n"
    ...
    "challenges harmonizing procurement systems following an acquisition. The integration "
    "required aligning disparate systems to streamline operations and achieve cost "
    "efficiencies... We implemented a unified procurement platform, enabling real-time data "
    "access and process automation... The client can now operate a synchronized procurement "
    "system, reducing redundancies.\" Capabilities like \"Real-Time Data Access Enablement\" "
    "and \"Resource Allocation Optimization\" are technology labels, not business functions. "
    "No results section. This is a vendor describing itself, not a proof point.\n"
    "GOOD (situation — write like this instead): \"A leading FMCG enterprise completed a "
    "significant acquisition but was running two entirely separate procurement ecosystems -- "
    "different ERPs, supplier databases, approval hierarchies and contract repositories with "
    "no connectivity. Teams duplicated supplier negotiations, raised POs in parallel systems, "
    "and reconciled spend manually at month-end. Supplier overlap went unmapped and contract "
    "leverage was left on the table.\" Solution: \"Consolidated two ERP environments into a "
    "single platform, standardizing supplier records, approval workflows and contract "
    "repositories. A supplier rationalization exercise ran alongside the technical "
    "integration. Unified spend analytics were live from day one of go-live.\" Results: "
    "\"Procurement systems consolidated from 2 ERP environments to 1, eliminating parallel "
    "spend tracking\"; \"Supplier base rationalized within 90 days of go-live, recovering "
    "negotiation leverage\"; \"Full enterprise spend visibility from day one, replacing "
    "month-end manual reconciliation.\" A reader sees their own problem reflected back at "
    "them, not a vendor describing itself.\n"
)
...
def case_from_content(content, context=None):
    ...
    prompt = (
        ...
        + "CASE STUDY CONTENT:\n\"\"\"\n" + (content or "")[:12000] + "\n\"\"\"\n\n"
        "TITLE — REUSE, DON'T REWRITE (owner-spec, 2026-08-12): if the pasted content already "
        "states a clear title or heading for this case study, use it VERBATIM (light "
        "capitalisation/punctuation cleanup only) — do not paraphrase a heading that's already "
        "good. EXCEPTION — named accelerator/tool: if the content names a specific, "
        "proper-noun-branded internal asset (e.g. 'Resolve AI', 'Auto-Ops', 'ClaimSense') as "
        "what was built or deployed, the title must be '<Name> - <what it does, plain "
        "language>' (the name, then a plain hyphen, then a short capability description) — "
        "even if the content's own heading is phrased differently, lead with the accelerator's "
        "name. Only when NEITHER a clear heading NOR a named accelerator is present should you "
        "write an original title, per the title rule in the field spec below.\n"
        "METRICS — REUSE, DON'T RE-DERIVE: if the content already states specific result "
        "metrics/numbers (a percentage, a count, a before/after figure), use those EXACT "
        "figures in the Results section, worded as given — do not round, restate as a "
        "different metric, or substitute a benchmark figure when a real one was provided. Only "
        "fall back to the RESULTS RULES' benchmark-inference allowance for a result the content "
...
def draft_case_study(brief, context=None):
    ...
    prompt = (
        ...
        "PROOF-number rule below, which governs the Challenge and Solution text "
        "specifically and is stricter than this general instruction (texture numbers -- team "
        "size, a rough duration, a volume count -- may still be invented realistically there).\n\n"
